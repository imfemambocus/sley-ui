...
A pointer costs the most in a dense table, so everything in one is reachable without it. Sorting, resizing a column, selection and the command palette all answer keys, and the docs page for it lists every binding I drove in a browser.

The chart is built on the same tokens as everything else. It reads the density knob and the palette, and it draws plain SVG, so your stylesheet reaches it and it needs no theme of its own. Drag across it to select a range and it hands the values back to you. That is how the demo on the home page narrows the table underneath it.
